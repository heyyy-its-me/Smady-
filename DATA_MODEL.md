...
| `proposal_review_log` | id (Integer, autoincrement), meeting_id, lead_email, proposal_json (JSONB), guardrail_errors, reviewer_approved, reviewer_issues, final_status, context_json, customer_id, user_id, created_at | N8N Proposal Agent workflow (writes proposals needing review) | Proposals page (review queue) |
| `proposal_results` | id, user_id, request_id, lead_name, lead_email, proposal_json (JSONB), guardrail_errors (JSONB), reviewer_approved, final_status | Legacy app-generated proposals | Proposals page (fallback/history) |
| `pricing_packages` | id (Integer), package_name, floor_price, ceiling_price, includes, valid_days, active | N8N Proposal Agent (read for guardrail checks) | Proposals page (pricing catalog display) |
